    ...
    <div className="space-y-10">
      ...
      <header className="rise">
        <div className="flex items-end gap-3">
          <span className="tnum text-mega font-bold text-[var(--aurora-1)]">
            +{ACTUAL_LIFT}
            <span className="text-title">%</span>
          </span>
        </div>
        <p className="mt-3 max-w-[30ch] text-body leading-snug text-[var(--frost-dim)]">
        ...
        <div className="mt-5 inline-flex items-center gap-2 rounded-full border border-[var(--gold)]/35 bg-[var(--gold)]/10 px-3 py-1">
          <span className="size-1.5 rounded-full bg-[var(--gold)]" />
          <span className="mono text-label text-[var(--gold)]">
      ...
      <section className="rise">
        ...
        <p className="mt-3 text-body text-[var(--frost-dim)]">
      ...
      <section className="rise">
        ...
        <p className="mt-3 mb-5 text-body text-[var(--frost-dim)]">
      ...
      <section className="rise">
        ...
        <button
          ...
          className="press hoverable mono mt-3 min-h-11 rounded-full border border-[var(--edge)] px-4 text-label text-[var(--frost-dim)]"
        >
        ...
        {showTable && (
          <div className="mt-3 overflow-x-auto">
            <table className="w-full border-collapse text-left">
              <thead>
                <tr className="mono text-label text-[var(--frost-faint)]">
              ...
              <tbody className="tnum text-meta">
                {RETENTION.map((r) => (
      ...
      <section className="rise pb-2">
        <h2 className="text-body text-[var(--frost-dim)]">Vanity metrics — not the point</h2>
        <div className="mt-3 flex flex-wrap gap-x-6 gap-y-2 opacity-45">
          {VANITY.map((v) => (
            <p key={v.label} className="text-meta text-[var(--frost-dim)]">
              <span className="tnum">{v.value}</span> {v.label.toLowerCase()}
            </p>
          ))}
        </div>
        <p className="mt-3 max-w-[38ch] text-meta text-[var(--frost-faint)]">
...
function SectionHead({ title }: { title: string }) {
  return <h2 className="text-head">{title}</h2>;
}

function Cell({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="pad-tight bg-[var(--night-1)]">
      <dd
        className={`tnum text-head leading-none font-bold ${
          accent ? "text-[var(--aurora-1)]" : "text-[var(--frost)]"
        }`}
      >
        {value}
      </dd>
      <dt className="mono mt-1.5 text-label leading-tight text-[var(--frost-faint)]">{label}</dt>
    </div>
  );
}
...
function Target({ hit, label, actual }: { hit: boolean; label: string; actual: string }) {
  return (
    <li className="flex items-start gap-3">
      <span
        aria-hidden
        className="mt-0.5 grid size-5 shrink-0 place-items-center rounded-full text-micro font-bold text-[var(--night-0)]"
      ...
      <span className="text-body">
        ...
        {label}
        <span className="mono tnum ml-2 text-label text-[var(--frost-faint)]">{actual}</span>
      </span>
    </li>
  );
}
